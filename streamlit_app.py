        ...
        if st.session_state['attended_mode'] and driver is not None:
            ...
            for i, url in enumerate(st.session_state['urls'], start=1):
                ...
                if show_tags:
                    ...
                    total_cost += cost
                    # Save formatted data
                    df = save_formatted_data(formatted_data, output_folder, f'sorted_data_{i}.json',
...
if st.session_state['scraping_state'] == 'completed' and st.session_state['results']:
    ...
    if pagination_info:
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.download_button("Download Pagination CSV", data=pagination_df.to_csv(index=False),
                               file_name="pagination_urls.csv")
        with col2:
            st.download_button("Download Pagination JSON", data=json.dumps(pagination_info['page_urls'], indent=4),
